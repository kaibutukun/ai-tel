...
@Injectable()
export class NttCpaasWebhookService {
  private readonly logger = new Logger(NttCpaasWebhookService.name);

  constructor(
    private readonly prisma: PrismaService,
    private readonly cpaasClient: NttCpaasClient
  ) {}
  ...
  private async handleCallReceived(event: NttCpaasEvent) {
    ...
    if (!to) {
      await this.cpaasClient.hangupCall(callId);
    ...
    if (phoneNumber.transferTo && !phoneNumber.callFlowId) {
      await this.cpaasClient.createDialog(callId, {
